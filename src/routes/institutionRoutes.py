...
async def get_institution_by_id(institution_id: int, db: AsyncSession = Depends(get_db)):
...
async def update_institution_endpoint(institution_id: int, institution: InstitutionUpdate, db: AsyncSession = Depends(get_db)):
    db_institution = await update_institution(db, institution_id, institution)
    if not db_institution:
        raise HTTPException(status_code=404, detail="Institución no encontrada")
    return db_institution

@INSTITUTION_ROUTES.delete("/institutions/{institution_id}", response_model=InstitutionResponse)
async def delete_institution_endpoint(institution_id: int, db: AsyncSession = Depends(get_db)):
