...
from src.schemas.trackingModeSchema import TrackingModeCreate, TrackingModeUpdate, TrackingModeResponse
...
@TRACKING_MODE_ROUTES.get("/tracking_modes/", response_model=list[TrackingModeResponse])
async def list_tracking_modes(db: AsyncSession = Depends(get_db)):
    return await get_tracking_modes(db)

@TRACKING_MODE_ROUTES.get("/tracking_modes/{tracking_mode_id}", response_model=TrackingModeResponse)
